...
class UnetPPDecoder(Model):
    ...
    def __init__(
        ...
        if se_module:
            self.layer4_1 = DecoderBlockSE(in_channels[0], out_channels[0], use_batchnorm=use_batchnorm)
            self.layer3_1 = DecoderBlockSE(encoder_channels[1] + encoder_channels[2], out_channels[1],
                                           use_batchnorm=use_batchnorm)
            self.layer3_2 = DecoderBlockSE(in_channels[1] + out_channels[1], out_channels[1],
                                           use_batchnorm=use_batchnorm)
            self.layer2_1 = DecoderBlockSE(encoder_channels[2] + encoder_channels[3], out_channels[2],
                                           use_batchnorm=use_batchnorm)
            self.layer2_2 = DecoderBlockSE(in_channels[2] + out_channels[2], out_channels[2],
                                           use_batchnorm=use_batchnorm)
            self.layer2_3 = DecoderBlockSE(in_channels[2] + out_channels[2] * 2, out_channels[2],
                                           use_batchnorm=use_batchnorm)
            self.layer1_1 = DecoderBlockSE(encoder_channels[3] + encoder_channels[4], out_channels[3],
                                           use_batchnorm=use_batchnorm)
            self.layer1_2 = DecoderBlockSE(in_channels[3] + out_channels[3], out_channels[3],
                                           use_batchnorm=use_batchnorm)
            self.layer1_3 = DecoderBlockSE(in_channels[3] + out_channels[3], out_channels[3],
                                           use_batchnorm=use_batchnorm)
            self.layer1_4 = DecoderBlockSE(in_channels[3] + out_channels[3] * 3, out_channels[3],
                                           use_batchnorm=use_batchnorm)
            self.layer0 = DecoderBlockSE(in_channels[4], out_channels[4], use_batchnorm=use_batchnorm)
            self.layer0_1 = DecoderBlockSE(in_channels[4], out_channels[4], use_batchnorm=use_batchnorm)
            self.layer0_2 = DecoderBlockSE(in_channels[4], out_channels[4], use_batchnorm=use_batchnorm)
            self.layer0_3 = DecoderBlockSE(in_channels[4], out_channels[4], use_batchnorm=use_batchnorm)
        else:
