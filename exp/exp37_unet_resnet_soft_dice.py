# ===============
# best_ckpt=13
# 2019-09-12 16:15:32,502 - INFO - Mean train loss: 0.00906
# 2019-09-12 16:15:52,897 - INFO - Mean valid loss: 0.0085
# ===============
import os
import gc
import time
...
from losses import SoftDiceLoss_binary
...
def main(seed):
    ...
    with timer('create model'):
        ...
        train_criterion = SoftDiceLoss_binary()
        criterion = torch.nn.BCEWithLogitsLoss()
        optimizer = torch.optim.Adam(model.parameters(), lr=3e-4)
        if base_model is None:
            scheduler_cosine = CosineAnnealingLR(optimizer, T_max=CLR_CYCLE, eta_min=3e-5)
            scheduler = GradualWarmupScheduler(optimizer, multiplier=1.1, total_epoch=CLR_CYCLE * 2,
    ...
    with timer('train'):
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            tr_loss = train_one_epoch(model, train_loader, train_criterion, optimizer, device, cutmix_prob=0.0)
