...
EPOCHS = 17
FOLD_ID = 0
EXP_ID = "exp8_unet_seresnext101"
model_path = "../input/sev-exp8-unetpp-seresnext101-se-fold0-6-lo2/exp8_unet_seresnext101_fold0_ckpt2.pth"
...
def main():
    ...
    with timer('create model'):
        model = smp.UnetPP('se_resnext101_32x4d', encoder_weights='imagenet', classes=N_CLASSES, encoder_se_module=True,
                         decoder_semodule=True, h_columns=False, deep_supervision=True)
    ...
    with timer('train'):
        ...
        for epoch in range(1, EPOCHS + 1):
            ...
            tr_loss = train_one_epoch_dsv(model, train_loader, criterion, optimizer, device)
            train_losses.append(tr_loss)
            LOGGER.info('Mean train loss: {}'.format(round(tr_loss, 5)))

            valid_loss, val_pred, y_val = validate_dsv(model, val_loader, criterion, device)
